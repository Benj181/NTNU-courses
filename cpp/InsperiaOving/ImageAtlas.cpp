...
//     static_assert(s.is_open());
//     for(line in s) { // loope gjennom lijene
//         int rank = line.split("\tb").at(0); // splitte den i tre og hente ut data
//         std::string suit = line.split("\tb").at(1);
//         std::string path = line.split("\tb").at(2);
//         // husker ikke syntex i cpp

//         std::string key{static_cast<std::string>(rank + 1) + suit};
//         std::string fullPath = static_cast<std::string>(IMAGE_DIR) + path
